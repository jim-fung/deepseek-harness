...
- **Profile 模块后备机制。** 裸插件 specifier 由 Loader 从配置目录解析。普通 Node 会为安装依赖闭包中的每个包维护一个符号链接。打包可执行文件无法让操作系统符号链接进入 pkg 的 `/snapshot` 树，因此会按 Node ESM 条件读取已安装包的 export map，并写入重新导出虚拟模块 URL 的真实代理包。缺失 export 保持不可用，错误 export map 会让启动失败，跨进程 writer lock 则会在不暴露部分代理的情况下替换陈旧条目。所选外部 bundle 若不在安装闭包中，则会获得 profile 本地的 `.dsh-module-fallback` 链接；已有 pnpm 条目优先，后续闭包发现会排除投影链接，清理也只删除 dsh 自有链接。共享后备机制背后的闭包遍历仅在该目录内的 `.dsh-heal.json` 印记不再匹配应用 manifest 与工作区 lockfile（各自的 mtime 与大小；lockfile 缺失只与缺失相等）或某条已记录条目不再最新时才执行；每次启动仍会核对已记录的各条目，印记读取、形状或写入失败一律回退到完整遍历，因此 healing 不会新增失败模式。手工修改了既不触及应用 manifest 也不触及 lockfile 的工作区 manifest 时，其变更要等到下一次完整遍历才会被拾取；在那之前 Loader 仍会响亮地拒绝无法解析的新插件，只是更晚且不够聚焦。
