...
function clippedNotice(omittedChars: number): string {
  return `<response clipped><NOTE>To save on context, ${omittedChars} characters were omitted from the middle of this command output; the beginning and end are shown. Re-run the command with output redirected to a file and search that file with Select-String to see the omitted part.</NOTE>\n`
}
