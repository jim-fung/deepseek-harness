...
Commands share one shell per agent, so state persists until an `exit`, a timeout, or a reset — each of which closes the shell and tells the agent the next call starts from the workspace with a fresh directory and environment. Results exclude the private completion markers; a non-zero wrapped command appends `[exit code: N]`, and a shell that exits before reporting that status instead appends `[shell exited: code N]`, `[shell killed by signal: SIG]`, or `[shell exited]`, then resets. Long output keeps equal head and tail halves within `maxOutputChars`, and the clipping notice between them names the exact number of omitted middle characters, so error and exit summaries at the end survive; if the terminal has already dropped the beginning, the result says so explicitly rather than presenting a tail as complete output.
