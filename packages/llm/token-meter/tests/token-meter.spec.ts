...
describe('replay anchors and surface folds', () => {
  ...
  it('folds later appends lazily: totals stay correct without any intermediate read', () => {
    ...
    expect(caught.logRevision).toBe(session.seq)
  ...
  it('does not rescan folded events on repeated reads', () => {
    ...
    // check: one `seq` access per call, never a traversal of the fold.
    const seqReads = vi.spyOn(Session.prototype, 'seq', 'get')
    const eventReads = vi.spyOn(Session.prototype, 'eventAt')
    const first = service.measure(session)
    const second = service.measure(session)
    const third = service.measure(session)
    expect(seqReads).toHaveBeenCalledTimes(3)
    expect(eventReads).not.toHaveBeenCalled()
    expect(second).toEqual(first)
    expect(third).toEqual(first)
    seqReads.mockRestore()
    eventReads.mockRestore()
  })
