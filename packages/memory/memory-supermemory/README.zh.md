---
description: "由 supermemory.ai 支持的 harness 记忆能力 seam 的 MemoryProvider：函数插件，向 ctx.memory 注册提供方 memory-provider:supermemory，不注册面向模型的工具。"
...
本包拥有凭据解析、容器标签 scope 映射与 wire 客户端；呈现其数据的工具与召回节归 dsh-tool-memory 所有。自动跳过的真实 API e2e（`tests/supermemory.e2e.ts`）是对外不固定 wire 字段的漂移告警。
