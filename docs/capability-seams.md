...
| `ctx.memory` | `seam` | [`memory`](../packages/memory/memory) | [`memory-supermemory`](../packages/memory/memory-supermemory) | [`tool-memory`](../packages/memory/tool-memory) | - | One active memory provider serves ctx.memory; tool-memory owns the model-facing tools and the recall section. |
