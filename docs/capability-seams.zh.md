...
flowchart LR
  ...
  pkg_memory["memory"]
  svc_memory["ctx.memory<br/>Persistent-memory provider seam"]
  ...
  pkg_memory_supermemory --> svc_memory
...
| `ctx.memory` | `seam` | [`memory`](../packages/memory/memory) | [`memory-supermemory`](../packages/memory/memory-supermemory) | [`tool-memory`](../packages/memory/tool-memory) | - | 一个激活的 memory 提供方为 ctx.memory 提供服务；tool-memory 拥有面向模型的工具与 recall 部分。 |
